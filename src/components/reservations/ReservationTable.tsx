...
interface ReservationTableProps {
  ...
  emptyMessage?: string;
  // Check-in físico (Fase 1) — solo tiene sentido para el propio socio
  // marcando su asistencia; el check-in de otro socio por STAFF/ADMIN
  // (walk-in sin reserva) queda fuera de esta pasada, ver CLAUDE.md.
  showCheckIn?: boolean;
  activeCheckIns?: Record<string, CheckIn>;
  onCheckInChange?: (reservationId: string, checkIn: CheckIn | null) => void;
}
...
export function ReservationTable({
  ...
  const handleCheckIn = async (reservation: Reservation) => {
    setCheckInLoadingId(reservation.id);
    setError('');
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        onCheckInChange?.(reservation.id, data);
      } else {
        setError(data.message || 'No se pudo registrar el check-in.');
      }
    } catch {
      setError('Error de conexión.');
    } finally {
  ...
  const handleCheckOut = async (reservation: Reservation, checkIn: CheckIn) => {
    setCheckInLoadingId(reservation.id);
    setError('');
    try {
      const res = await apiFetch(`/checkins/${checkIn.id}/checkout`, { method: 'POST' });
      const data = await res.json();
      if (res.ok) {
        onCheckInChange?.(reservation.id, data);
      } else {
        setError(data.message || 'No se pudo registrar el check-out.');
      }
    } catch {
      setError('Error de conexión.');
    } finally {
        ...
        <table className="min-w-full text-sm text-left text-cream-muted">
          <thead className="text-xs uppercase bg-ink-950 text-cream-muted font-semibold border-b border-ink-line-strong">
            <tr>
              ...
              {(canCancel || showCheckIn) && <th className="px-6 py-4 text-right">Acciones</th>}
            </tr>
          </thead>
          <tbody className="divide-y divide-ink-line">
            {reservations.length > 0 ? (
              reservations.map((r) => {
                const activeCheckIn = activeCheckIns[r.id];
                const canShowCheckIn =
                  showCheckIn && r.status === 'confirmed' && isToday(r.startAt);
                return (
                  <tr key={r.id} className="hover:bg-ink-950 transition-colors">
                    ...
                    {(canCancel || showCheckIn) && (
                      <td className="px-6 py-4 text-right space-x-3 whitespace-nowrap">
                        {canShowCheckIn &&
                          (activeCheckIn ? (
                            <button
                              onClick={() => handleCheckOut(r, activeCheckIn)}
                              disabled={checkInLoadingId === r.id}
                              className="text-warn hover:text-warn disabled:opacity-70 font-semibold transition-colors"
                            >
                              {checkInLoadingId === r.id ? 'Guardando...' : 'Check-out'}
                            </button>
                          ) : (
                            <button
                              onClick={() => handleCheckIn(r)}
                              disabled={checkInLoadingId === r.id}
                              className="text-success hover:text-success disabled:opacity-70 font-semibold transition-colors"
                            >
                              {checkInLoadingId === r.id ? 'Guardando...' : 'Check-in'}
                            </button>
                          ))}
              ...
              <tr>
                <td
                  colSpan={canCancel || showCheckIn ? 4 : 3}
                  className="px-6 py-12 text-center text-cream-faint"
                >
